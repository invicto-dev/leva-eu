...
export class WhatsappService implements OnModuleInit {
  ...
  private async logMessage(phone: string, body: string, type: 'RECEIVED' | 'SENT') {
  ...
  public async handleIncomingMessage(msg: { from: string; body: string; reply: (text: string) => Promise<any> }) {
    ...
    if (text === '!cancelar') {
      ...
      return this.sendReply(msg as Message, 'Sua solicitação atual foi cancelada. Se precisar de algo, é só chamar!');
    }

    if (text === '!ajuda') {
      return this.sendReply(msg as Message, 'Comandos disponíveis:\n*!cancelar* - Cancela a solicitação atual\n*!ajuda* - Mostra esta mensagem');
    }
    ...
    switch (state) {
      case 'IDLE':
        if (text.includes('corrida') || text.includes('oi') || text.includes('quero')) {
          ...
          await this.sendReply(msg as Message, 'Olá! Eu sou o Leva Eu. 🚗\nPara começar, qual o seu local de **ORIGEM**?');
        } else {
          await this.sendReply(msg as Message, 'Olá! Digite "Quero uma corrida" para começar ou *!ajuda* para mais opções.');
        }
        break;

      case 'ASKING_ORIGIN':
        await this.updateConversationState(customer.id, 'ASKING_DESTINATION', { origin: msg.body });
        await this.sendReply(msg as Message, 'Entendido! Agora, para onde você deseja ir? (**DESTINO**)');
        break;
      ...
      case 'CONFIRMING':
        if (text === 'sim') {
          if (customer.state && customer.state.tempData) {
            ...
            await this.ridesService.createRide({ customerId: customer.id, origin, destination });
            await this.updateConversationState(customer.id, 'IDLE');
            await this.sendReply(msg as Message, 'Sua corrida foi solicitada! 🚀\nEstamos procurando o motorista mais próximo.');
          }
        } else if (text === 'cancelar') {
          await this.updateConversationState(customer.id, 'IDLE');
          await this.sendReply(msg as Message, 'Solicitação cancelada.');
        } else {
          await this.sendReply(msg as Message, 'Por favor, responda *SIM* para confirmar ou *CANCELAR*.');
        }
        break;
    }
  }
  ...
  private async updateConversationState(customerId: string, step: string, tempData: any = null) {
    await this.prisma.conversationState.upsert({
      where: { customerId },
      update: { step: step as any, tempData: tempData ? tempData : undefined },
      create: { customerId, step: step as any, tempData: tempData ? tempData : {} },
    });
  }
}
